...
/**
  ...
  * @param  id: connection to be checked
...
check_connection(uint8_t conid)
{
	if(conid>at_linkMax-1)
		return -1;
	else
	if(pLink[conid].linkEn)
		return 1;
...
//    return;
//  }
  if((wifi_station_get_connect_status() != STATION_GOT_IP) && (at_wifiMode == 1 ))
  {
    uart0_sendStr("no ip\r\n");
    return;
  }
...
//HACK: This need to be somewhere else as it is no AT+Command

/**
  * @brief  Send data directly to a connectionID
...
/**
  * @brief  Get a pointer to a connection
  * @param  id: connectionID
  * @retval Pointer to connection
  */
at_linkConType* ICACHE_FLASH_ATTR
get_con(uint8_t conID)
{
	//TEST
	char temp[128];
	os_sprintf(temp,"\r\n   Connection #%d is: %s     \r\n",pLink[conID].linkId, pLink[conID].linkEn);
	uart0_sendStr(temp);
